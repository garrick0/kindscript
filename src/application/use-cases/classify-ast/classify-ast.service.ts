...
import { GENERATORS } from '../check-contracts/generator-registry';
import { hasIntrinsicPure, propagatePurity } from '../check-contracts/purity/purity.generator';
 ...
 * intrinsic constraints (e.g., pure: true) from leaf Kinds are automatically
 * applied to composites that contain them.
 *
...
export class ClassifyASTService implements ClassifyASTUseCase {
  ...
  execute(request: ClassifyASTRequest): ClassifyASTResponse {
    ...
    for (const sourceFile of request.definitionFiles) {
      ...
      const instanceViews = this.astPort.getInstanceDeclarations(sourceFile);
      for (const view of instanceViews) {
        ...
        // Root is the directory containing the .k.ts file
  ...
  private buildMemberTree(
    ...
    for (const property of kindDef.members) {
      ...
      if (memberValue?.pathOverride) {
        pathSegment = memberValue.pathOverride;
      }
   ...
   * Also propagates intrinsic constraints (e.g., pure) from leaf Kinds to composites.
   */
    ...
    for (const [kindName, kindDef] of kindDefs) {
      ...
      for (const instanceSymbol of instances) {
        ...
        for (const prop of kindDef.members) {
          ...
          if (!memberKindDef?.constraints || !hasIntrinsicPure(memberKindDef.constraints)) continue;

          // Auto-generate purity contract for this member
          const memberSymbol = instanceSymbol.findByPath(prop.name);
          if (memberSymbol) {
            const contract = propagatePurity(
   ...
   * Uses GENERATORS registry to map dotted property names to generators.
  ...
  private walkConstraints(
    ...
    for (const prop of view.properties) {
      ...
      // Look up generator
      const entry = GENERATORS[fullName];
      if (!entry) {
      ...
      if (entry.intrinsic) continue;

      const result = entry.generate(value, instanceSymbol, kindName, location);
