import { MustImplementChecker } from '../../src/application/use-cases/check-contracts/must-implement/must-implement.checker';
import { generateMustImplement } from '../../src/application/use-cases/check-contracts/must-implement/must-implement.generator';
...
describe('MustImplementChecker', () => {
  let checker: MustImplementChecker;
  ...
  beforeEach(() => {
    ...
    checker = new MustImplementChecker();
  });
  ...
  it('handles mustImplement with no declared location', () => {
    ...
    const result = checker.check(mustImplement(noLoc, withLoc), makeContext());
  ...
  it('detects missing implementation', () => {
    ...
    const result = checker.check(mustImplement(ports, adapters), makeContext(resolvedFiles));
  ...
  it('handles missing locations', () => {
    ...
    const result = checker.check(mustImplement(ports, adapters), makeContext());
    expect(result.diagnostics).toHaveLength(0);
...
describe('generateMustImplement', () => {
  it('returns empty for wrong shape', () => {
    const instance = makeSymbol('inst');
    const result = generateMustImplement(
