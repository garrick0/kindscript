import { NoDependencyChecker } from '../../src/application/use-cases/check-contracts/no-dependency/no-dependency.checker';
import { generateNoDependency } from '../../src/application/use-cases/check-contracts/no-dependency/no-dependency.generator';
...
describe('NoDependencyChecker', () => {
  let checker: NoDependencyChecker;
  ...
  beforeEach(() => {
    ...
    checker = new NoDependencyChecker();
  });
  ...
  it('detects forbidden dependency from domain to infrastructure', () => {
    ...
    const result = checker.check(noDependency(domain, infra), makeContext(resolvedFiles));
  ...
  it('handles symbols with no declared location', () => {
    ...
    const result = checker.check(noDependency(noLoc, withLoc), makeContext());
  ...
  it('handles symbol with no declared location', () => {
    ...
    const result = checker.check(noDependency(domain, infra), makeContext());
    expect(result.diagnostics).toHaveLength(0);
  });
...
describe('generateNoDependency', () => {
  it('returns empty for wrong shape', () => {
    const instance = makeSymbol('inst');
    const result = generateNoDependency(
