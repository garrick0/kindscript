...
export function LoadingOverlay({ state }: LoadingOverlayProps) {
  ...
  useEffect(() => {
    ...
    const targetDuration = 45000; // 45 seconds expected duration
    ...
    const interval = setInterval(() => {
      ...
      const percentage = Math.min(95, (elapsed / targetDuration) * 90 + Math.log(elapsed / 1000 + 1) * 5);
  ...
  useEffect(() => {
    ...
    const startTime = Date.now();
    const targetDuration = 3000; // 3 seconds expected

    const interval = setInterval(() => {
      const elapsed = Date.now() - startTime;
      setElapsedTime(Math.floor(elapsed / 1000));
      const percentage = Math.min(90, (elapsed / targetDuration) * 90);
      setProgress(Math.round(percentage));
    }, 100);
  ...
  const estimatedRemaining = state === 'installing' ? Math.max(0, 45 - elapsedTime) : 0;
