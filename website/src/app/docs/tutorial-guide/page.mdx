...
Kind classifies **directories**. Wrapped Kinds classify **types** — individual exported declarations within a directory.
...
Wrapped Kinds use `Kind` with the `wraps` config to add architectural meaning to TypeScript types:
...
Functions declare their role via type annotation using `InstanceOf<K>`:
...
| `Kind<N, {}, C, { wraps: T }>` | Wraps a TypeScript type with architectural meaning (wrapped Kind) |
| `InstanceOf<K>` | Annotates exports as instances of a wrapped Kind |
...
- [Kind System](02-kind-system.md) — full Kind and wrapped Kind reference
