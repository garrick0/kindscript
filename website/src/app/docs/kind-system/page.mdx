...
A Kind is a TypeScript type alias using the `Kind<N, Members, Constraints, Config>` generic. `Kind` is a conditional type — when `Config` includes `wraps`, it produces a wrapped type (declaration-level enforcement); otherwise it produces a structural type for `satisfies Instance<T, Path>`.
...
| `_Config` | No | `KindConfig` — `{ wraps?: T; scope?: 'folder' \| 'file' }`. `wraps` makes Kind produce a type annotation shape (declaration-level enforcement). `scope` declares the expected instance scope, validated by the scope plugin. |
...
Where a plain Kind classifies **places** (directories and files), a wrapped Kind classifies **types** (individual exported declarations via their type annotations).

Wrapped Kinds use the `wraps` property in Kind's `Config` parameter: `Kind<N, {}, C, { wraps: T }>`.
...
Wrapped Kinds take the same four type parameters as Kind:
- `N` — the kind name (string literal)
- `Members` — always `{}` (wrapped Kinds have no members)
- `C` — optional constraints (same as Kind's 3rd parameter)
- `Config` — `{ wraps: T }` where `T` is the wrapped TypeScript type

The implementation uses a phantom brand: `T & { readonly __kindscript_brand?: N }`. The brand is optional, so any value of type `T` satisfies the wrapped Kind — zero assignability impact.

### Wrapped Kind Instances

The type annotation IS the instance declaration. Use `InstanceOf<K>` to annotate exports:
...
Discovery is **syntactic** — the scanner checks whether the type annotation explicitly uses `InstanceOf<K>` where `K` is a wrapped Kind, not whether the value structurally matches. The developer's choice of type annotation is the opt-in signal.
...
### Wrapped Kind vs Kind

| Aspect | Kind | Wrapped Kind |
|--------|------|----------|
| Classifies | Places (directories, files) | Types (exported declarations) |
| Scope | Directory or file | Individual declaration |
| Instance mechanism | `satisfies Instance<T, Path>` | Type annotation with `InstanceOf<K>` |
...
| Underlying type | `Kind<N, Members, C, Config>` | `Kind<N, {}, C, { wraps: T }>` |
...
| `InstanceOf<K>` | Annotate exports as instances of a wrapped Kind (declaration-level enforcement) |
