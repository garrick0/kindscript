...
```
  ┌──────────────────────────────────────────────────────────────────┐
  │  v2 architecture (correct — mirrors TypeScript)                   │
  │                                                                    │
  │   kind-definitions.ts ──┐                                         │
  ...
  │                  └──────────────────────┘          Diagnostics     │
  │                                                                    │
  │   One parser. One AST. Host queried on demand.                    │
  └──────────────────────────────────────────────────────────────────┘
```
...
```
  KS Binder: AST Classification
  ──────────────────────────────

  Walking the TypeScript AST...

  ┌────────────────────────────────────────┐
  │  type OrderingContext = Kind<"..."> &  │
  │    { domain: DomainLayer; ... }        │──→ ArchSymbol (symbolKind: Kind)
  └────────────────────────────────────────┘     with members: domain, app, infra

  ┌────────────────────────────────────────┐
  │  const ordering: OrderingContext = {   │
  │    kind: "OrderingContext",            │──→ ArchSymbol (symbolKind: Instance)
  │    location: "src/contexts/ordering",  │     linked to Kind symbol above
  │    domain: { ... }                     │
  │  }                                     │
  └────────────────────────────────────────┘
  ...
  ┌────────────────────────────────────────┐
  │  const contracts =                     │
  │    defineContracts<OrderingContext>({   │──→ Contract[] attached to Kind symbol
  │      noDependency: [["domain","infra"]]│
  │    })                                  │
  └────────────────────────────────────────┘

  ┌────────────────────────────────────────┐
  │  import { Db } from '../infra/db'      │──→ recorded as dependency edge
  └────────────────────────────────────────┘     (fact, queried via host)
```
...
```
  ...
  └────┬─────┘                        └────┬─────┘     architecturally)
       │                                   │
   ts.Symbol                          ArchSymbol     ← NEW type
       │                                   │
  ┌────▼─────┐                        ┌────▼─────┐
  │TS Checker│                        │KS Checker│  ← NEW (contracts)
  └────┬─────┘                        └────┬─────┘
       │                                   │
  ...
  Host:   queried on demand by binder + checker in both systems
```
...
are three things: a binder, a checker, and a host.
...
```
  ...
  VarDecl ───────────────────────────────→  ArchSymbol
    ...
    initializer: { ... }                      kind: → (ref to Kind above)

  CallExpression ────────────────────────→  Contract[]
    defineContracts<OrderingContext>({         noDependency(domain, infra)
      noDependency: [["domain","infra"]]      mustImplement(ports, adapters)
    })                                        attached to Kind symbol
```

The binder's four responsibilities:
...
```
  Contract evaluation flow (noDependency example):

  ┌───────────────────────────────────────┐
  │  contract: noDependency(domain, infra) │
  └───────────────────┬───────────────────┘
                      │
                      ▼
  ┌───────────────────────────────────────┐
  │  resolve "domain" → ArchSymbol        │
  │    location: src/ordering/domain/      │
  │                                        │
  │  resolve "infra" → ArchSymbol          │
  │    location: src/ordering/infra/       │
  └───────────────────┬───────────────────┘
                      │
                      ▼
  ┌───────────────────────────────────────┐
  │  host.getImportsForFile(              │
  │    each file in domain/               │
  │  )                                     │
  └───────────────────┬───────────────────┘
                      │
  ...
  │    does it resolve to a file           │
  │    inside infra/?                      │
  │                                        │
  │    YES → ArchDiagnostic (KS1001)       │
  │    NO  → ok, continue                  │
  └───────────────────────────────────────┘
```
...
```
  ...
  │                    │   │                    │   │                    │
  │  Real filesystem   │   │  Wraps another     │   │  In-memory files   │
  │  Real TS compiler  │   │  host with cache   │   │  Predetermined     │
  │  Real package.json │   │  layer. Hashes     │   │  import edges.     │
  │                    │   │  files. Persists    │   │  Controlled dirs.  │
  │  Used in           │   │  to .ksbuildinfo   │   │                    │
  │  production.       │   │                    │   │  No filesystem     │
  │                    │   │  Used in watch +   │   │  needed.           │
  │                    │   │  incremental mode. │   │                    │
  └──────────────────┘   └──────────────────┘   └──────────────────┘
```
...
```
  ...
  ┌─────────────────────┐         ┌──────────────────────────────┐
  │  Existing codebase   │────────→│  Inferred kind definitions:   │
  │  (recognized layout) │  infer  │                                │
  │                      │         │  type Inferred = Kind<"...">   │
  │                      │         │    & { domain: ...; ... }      │
  └─────────────────────┘         └──────────────────────────────┘
```
...
```
  ...
  ║   ┌───────────────────────▼───────────────────────────┐    ║
  ║   │                                                     │    ║
  ║   │   KindScriptHost ◄──── KS Binder ────→ KS Checker  │    ║
  ║   │                        ArchSymbols[]                │    ║
  ║   └───────────────────────────────────────────────────┘    ║
  ║                           │                                  ║
  ║   getDiagnostics() ──────▼───────────────────────────────  ║
...
```
  ...
  │  • which arch symbols are still valid                         │
...
```
  Incremental re-check flow:

  File changed: src/ordering/domain/service.ts
       │
       ▼
  ┌─────────────────────────────────────┐
  │  1. Compare hash against cache       │
  │     hash(service.ts) ≠ cached hash   │
  │     → file is dirty                  │
  └──────────────────┬──────────────────┘
                     │
                     ▼
  ┌─────────────────────────────────────┐
  │  2. Invalidate cached host results   │
  │     for service.ts:                  │
  │     - import edges                   │
  │     - exported symbols               │
  └──────────────────┬──────────────────┘
                     ...
                     ▼
  ┌─────────────────────────────────────┐
  │  3. Propagate through dep graph      │
  │     files that import service.ts     │
  │     may also need re-checking        │
  └──────────────────┬──────────────────┘
                     │
                     ▼
  ┌─────────────────────────────────────┐
  │  4. Re-run only affected contracts   │
  │     noDependency(domain, infra)      │
  │       → re-check (domain changed)    │
  │     noCycles(contexts)               │
  │       → re-check (edges changed)     │
  │     purity(domain)                   │
  │       → re-check (imports changed)   │
  │     mustImplement(ports, adapters)   │
  │       → skip (no port/adapter change)│
  └─────────────────────────────────────┘
```
...
```
  ...
  │                                                                  │
  │  ┌─────────────────┐   ┌──────────────────────────────────┐    │
  │  │ .ts definition   │   │ target codebase files              │    │
  │  │ files changed    │   │ changed                            │    │
  │  └────────┬────────┘   └───────────────┬──────────────────┘    │
  │           │                             │                        │
  │           ▼                             ▼                        │
  │  re-parse definitions          invalidate host cache             │
  │  re-bind (reclassify           for changed files                 │
  │   arch symbols)                         │                        │
  │           │                             │                        │
  │           └──────────┬──────────────────┘                        │
  │                      ▼                                           │
  ...
  │                                                                  │
  │  ┌──────────────────────────────────────────────────────────┐   │
  │  │ Filesystem structure changes (new/deleted files/dirs)     │   │
  │  │ → invalidate directory listings                           │   │
  │  │ → re-check location contracts                             │   │
  │  └──────────────────────────────────────────────────────────┘   │
  └───────────────────────────────────────────────────────────────┘
```
...
```
  ...
  │  ─────            ──────          ───────          ────────────── │
  │  KindScriptHost   Walk TS AST    noDependency     ks.createProg  │
  │  defaultHost      → ArchSymbols  location check   CLI entry point│
  │  testHost         find Kind<N>   + incremental:   diagnostic fmt │
  │                   find instances mustImplement                    │
  │                   find contracts noCycles                         │
  │                                  purity                          │
  │                                                                   │
  ...
  │                                                                   │
  │  Emitter                Language Service       Incremental        │
  │  ───────                ────────────────       ───────────        │
  │  scaffold from kinds    extend TS LangSvc     cachedHost          │
  │  fix patches            arch diagnostics      .ksbuildinfo        │
  │  infer kinds            quick fixes           file watchers       │
  │                         code lens                                 │
  │                                                                   │
  │  ◄── generates code ──► ◄── IDE experience ──► ◄── fast ────────►│
  └─────────────────────────────────────────────────────────────────┘
```
...
```
  ...
  │   │   (classify AST                   (evaluate contracts    │      │
  │   │    → ArchSymbols)                  → ArchDiagnostics)    │      │
  │   │                                                          │      │
  │   └─────────────────────────────────────┬──────────────────┘      │
  │                                         │                          │
  │   ┌─────────────────────────────────────▼──────────────────┐      │
  ...
  │   │   (TS + KS       scaffolds     quick fixes               │      │
  │   │    merged)        patches       hover info               │      │
  │   └──────────────────────────────────────────────────────────┘      │
  │                                                                    │
  └──────────────────────────────────────────────────────────────────┘
```
